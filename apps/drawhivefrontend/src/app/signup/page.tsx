...
interface Form { name: string; email: string; password: string ,username:string}
...
export default function SignupPage() {
  const [form, setForm] = useState<Form>({ name: "", email: "", password: "" ,username:""});
              ...
              {!done ? (
                <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <motion.div {...fade(0.05)} className="mb-5">
                    <p className="font-caveat text-xs tracking-widest text-stone-400 mb-1">CREATE ACCOUNT</p>
                  ...
                  {/* Fields */}
                  <motion.div {...fade(0.2)}>
                    <Field label="Username ✍️" placeholder="ada@user" value={form.username} onChange={set("username")}  />
                    <Field label="Name ✍️" placeholder="Ada Lovelace" value={form.name} onChange={set("name")}  />
                  ...
                  <motion.p {...fade(0.3)} className="mt-3 text-center font-caveat text-sm text-stone-400">
                    ...
                    <span className="text-amber-600 font-semibold underline decoration-wavy cursor-pointer">Log in →</span>
                  </motion.p>
                </motion.div>
              ) : (
                <motion.div
                ...
                >
                  ...
                  >🎉</motion.div>
                  <h2 className="font-caveat text-2xl font-bold text-stone-800">You&apos;re in!</h2>
                  <p className="font-caveat text-stone-400 mt-1">Welcome, {form.name || "friend"} ✨</p>
